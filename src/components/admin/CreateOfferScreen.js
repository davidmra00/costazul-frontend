...
import { offerCreateOfferAsync, offerLoadOffersAsync, offerUpdateOfferAsync, offerUploadOfferPicture } from '../../actions/offerActions';
...
const CreateOfferScreen = () => {
  ...
  useEffect(() => {
    if(offer){
      reset({
        precio:offer.precio,
        desde:offer.fechaInicio.slice(0,10),
        hasta:offer.fechaFin.slice(0,10),
        lugar:offer.lugar,
        descripcion:offer.descripcion
      });
      setDisponible(offer.disponibilidad);
      setIncluye(offer.incluye);
      setNoIncluye(offer.noIncluye);
      setPhotos(offer.fotoUrl);
    }else{
      reset();
      setIncluye(['']);
      setNoIncluye(['']);
  ...
  const handleFile = (e, index) => {
    ...
    dispatch(offerUploadOfferPicture(formData)).then(url => handleChangePhoto(url, index));
  }
  ...
  const handleSave=(e)=>{
    ...
    if(offer){
      if (isValid()) {
        ...
        dispatch(offerUpdateOfferAsync( id,disponible,precio,desde,hasta,lugar,descripcion,include,notInclude,photos));
      ...
      if (isValid()) {
        ...
        const include=incluye.filter(Boolean);
        const notInclude=noIncluye.filter(Boolean);
        dispatch(offerCreateOfferAsync(disponible,precio,desde,hasta,lugar,descripcion,include,notInclude,photos));
        reset();
        setPhotos([]);
        setDisponible(false);
      }
